<script setup lang="ts">
  ...
  const parametersText = computed({
    get: () => {
      try {
        return JSON.stringify(formData.value.parameters ?? {}, null, 2);
      } catch {
        return '{}';
      }
    },
  ...
  const selectedFunction = computed(() => {
    if (!formData.value.function_kvid) return null;
    return allFunctions.value.find(item => item.kvid === formData.value.function_kvid) ?? null;
  });

  const renderTypeLabel: Record<FunctionRow['render_type'], string> = {
    webview: 'WebView',
    vue: 'Vue',
    umd: 'UMD',
  };

  const renderTypeCls: Record<FunctionRow['render_type'], string> = {
  ...
  async function saveMenu() {
    ...
    if (
      (formData.value.type === 'Page' || formData.value.type === 'Link') &&
      !formData.value.function_kvid
    ) {
      alert('Page / Link 类型菜单必须绑定功能');
      return;
    }
    saving.value = true;
    try {
      const funcKvid =
        formData.value.type === 'Page' || formData.value.type === 'Link'
          ? (formData.value.function_kvid ?? null)
          : null;
  ...
  async function deleteRoot() {
    if (!selectedRoot.value) return;
    if (!confirm(`确定删除根节点「${selectedRoot.value.title}」及其所有菜单？此操作不可恢复。`))
      return;
    const { error } = await adminSupabase
      .from('menu_roots')
      .delete()
      .eq('kvid', selectedRoot.value.kvid);
    if (error) {
...
<template>
  <div class="flex flex-col gap-4">
    ...
    <!-- Root Bar -->
    <div
      class="flex items-center gap-3 bg-white dark:bg-gray-800 rounded-xl px-4 py-3 shadow-sm flex-shrink-0"
    >
      ...
      >
        <option
          v-if="menuRoots.length === 0"
          :value="null"
        >
          暂无根节点，请先新建
        </option>
        <option
          v-for="root in menuRoots"
          :key="root.kvid"
          :value="root.kvid"
        >
    ...
    >
      ...
      <div class="w-80 flex-shrink-0 bg-white dark:bg-gray-800 rounded-xl shadow-sm flex flex-col">
        <div
          class="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex-shrink-0"
        >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar py-1">
          ...
          <div
            v-else-if="loading"
            class="flex items-center justify-center h-20 text-gray-400 text-sm gap-2"
          >
            <i class="fas fa-spinner fa-spin"></i>加载中
          </div>
          <div
            v-else-if="renderList.length === 0"
            class="flex flex-col items-center justify-center h-32 text-gray-400 text-sm gap-2"
          >
          ...
          >
            ...
            >
              ...
              <span
                v-else
                class="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600 block"
              ></span>
            </button>
            ...
            <span
              v-if="!node.is_active"
              class="text-[9px] text-gray-400 flex-shrink-0 opacity-0 group-hover:opacity-100"
            >
              OFF
            </span>
            ...
            <!-- Action buttons -->
            <div
              class="flex items-center flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
              @click.stop
            >
              <button
        ...
        <template v-else>
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <span
                v-if="panelMode === 'edit' && formData.kvid"
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                <label class="form-label"
                  >显示名称 <span class="text-gray-400 font-normal">(可选，覆盖标题)</span></label
                >
                <input
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  ...
                  <i
                    v-if="formData.icon"
                    :class="formData.icon"
                    class="text-gray-400 text-base w-5 text-center flex-shrink-0"
                  ></i>
                </div>
              </div>
              <div>
            ...
            <div>
              <label class="form-label">
                备注
                <span
                  v-if="formData.type === 'System'"
                  class="text-yellow-500 font-normal"
                  >— System 类型时此字段为路由路径</span
                >
              </label>
              <input
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                >
                  <i
                    :class="
                      formData.is_active
                        ? 'fas fa-toggle-on text-green-500 text-lg'
                        : 'fas fa-toggle-off text-gray-400 text-lg'
                    "
                  ></i>
              ...
              <div>
                <label class="form-label"
                  >AutoStartup <span class="text-gray-400 font-normal">(首页自动打开)</span></label
                >
                <button
                  @click="toggleAutoStartup"
                  class="flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition-colors w-full"
                  :class="
                    formData.parameters?.AutoStartup
                ...
                >
                  <i
                    :class="
                      formData.parameters?.AutoStartup
                        ? 'fas fa-toggle-on text-blue-500 text-lg'
                        : 'fas fa-toggle-off text-gray-400 text-lg'
                    "
                  ></i>
            ...
            >
              <div class="flex items-center justify-between">
                ...
                >
                  ...
                  <span
                    v-if="formData.function_kvid"
                    class="text-xs font-mono text-gray-400"
                    >{{ formData.function_kvid }}</span
                  >
                </h4>
                <button
                  v-if="formData.function_kvid"
                  @click="formData.function_kvid = null"
                  class="text-xs px-2.5 py-1 rounded-lg transition-colors text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20"
                >
                  清除绑定
                </button>
              </div>
              ...
              <div>
                <label class="form-label">
                  ...
                  <span class="font-normal text-gray-400"
                    >— 菜单只负责组合功能，不再直接填写 handler</span
                  >
                </label>
                <select
                  v-model="formData.function_kvid"
                  class="form-input"
                >
                  <option :value="null">请选择功能</option>
                  <option
                    v-for="func in allFunctions"
                    :key="func.kvid"
                    :value="func.kvid"
                  >
                    {{ func.title || func.handler }}
                    ·
                    {{ renderTypeLabel[func.render_type] }}
                    {{ func.is_active ? '' : '（已禁用）' }}
                  </option>
                </select>
              </div>
              ...
              >
                <div class="flex items-center gap-3">
                  <div
                    class="w-9 h-9 rounded-lg flex items-center justify-center bg-blue-100 dark:bg-blue-900/30"
                  >
                    <i
                      :class="selectedFunction.icon || 'fas fa-puzzle-piece'"
                      class="text-blue-600 dark:text-blue-400 text-sm"
                    />
                  </div>
                  <div class="min-w-0">
                    <div class="font-medium text-gray-800 dark:text-white truncate">
                      {{ selectedFunction.title || selectedFunction.handler }}
                    </div>
                    <div class="text-xs font-mono text-gray-400 truncate">
                      {{ selectedFunction.kvid }}
                    </div>
                  </div>
                  <span
                    class="ml-auto text-xs px-2 py-0.5 rounded font-medium"
                    :class="renderTypeCls[selectedFunction.render_type]"
                  >
                    {{ renderTypeLabel[selectedFunction.render_type] }}
                  </span>
                </div>

                <div class="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <div class="text-gray-400 mb-1">Handler</div>
                    <div class="font-mono text-gray-700 dark:text-gray-300 break-all">
                      {{ selectedFunction.handler }}
                    </div>
                  </div>
                  <div>
                    <div class="text-gray-400 mb-1">组件名</div>
                    <div class="font-mono text-gray-700 dark:text-gray-300 break-all">
                      {{ selectedFunction.source_component || '—' }}
                    </div>
                  </div>
                  <div>
                    <div class="text-gray-400 mb-1">来源模块</div>
                    <div class="font-mono text-gray-700 dark:text-gray-300 break-all">
                      {{ selectedFunction.source_module || '—' }}
                    </div>
                  </div>
                  <div>
                    <div class="text-gray-400 mb-1">来源地址</div>
                    <div class="font-mono text-gray-700 dark:text-gray-300 break-all">
                      {{ selectedFunction.source_url || selectedFunction.remark || '—' }}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <!-- Panel Footer -->
          <div
            class="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex-shrink-0"
          >
            <button
              v-if="panelMode === 'edit'"
              @click="deleteMenu"
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md m-4 overflow-hidden"
        >
          <div
          ...
          <div class="p-6 space-y-4">
            <div>
              <label class="form-label">标题 <span class="text-red-400">*</span></label>
              <input
                v-model="rootForm.title"
                type="text"
                class="form-input"
                placeholder="GavinYin Dashboard"
              />
            </div>
            <div>
              <label class="form-label">显示名称</label>
              <input
                v-model="rootForm.display_name"
                type="text"
                class="form-input"
              />
            </div>
            <div>
              ...
              <input
                v-model="rootForm.internal_code"
                type="text"
                class="form-input font-mono"
                placeholder="umdDashboard"
              />
            </div>
            <div>
